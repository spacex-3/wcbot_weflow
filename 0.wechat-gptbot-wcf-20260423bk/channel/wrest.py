...
from utils.serialize import serialize_video, serialize_img, should_download_remote_media, xml_to_dict, get_value
    ...
    def get_refer_image(self, refermsg: dict, save_dir=None):
        ...
        for attempt in range(max_retries):
            try:
                ...
                if 'error' in res:
                    logger.error('Attempt %d: Got refer image failed: %s', attempt + 1, [dat, res])
                else:
                    return res.get('result')
            except Exception as e:
                logger.error(f'Attempt {attempt + 1}: Exception during image download: {e}')
